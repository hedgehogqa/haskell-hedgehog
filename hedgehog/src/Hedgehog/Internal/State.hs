...
import           Hedgehog.Internal.Property (TestT, evalEither, evalM, success, runTestT, failWith, annotate)
...
--
-- This is the main type you will use when writing state machine tests. @gen@ is
-- usually an instance of 'MonadGen'. These constraints appear when you pass
...
data Action m (state :: (* -> *) -> *) =
  ...
  Action {
    ...
    , actionEnsure ::
        state Concrete -> state Concrete -> input Concrete -> output -> TestT m ()
    }
...
data ActionCheck m state =
  ActionCheck {
      checkUpdate :: state Concrete -> state Concrete
    , checkEnsure :: state Concrete -> state Concrete -> TestT m ()
    }

execute :: (HasCallStack, Monad m) => Action m state -> StateT Environment (TestT m) (ActionCheck m state)
...
executeUpdateEnsure ::
  ...
  -> Action m state
  -> TestT m (state Concrete, Environment)
...
successful :: Monad m => TestT m () -> m Bool
successful x = do
  (result, _) <- runTestT x
...
checkActions :: Monad m => state Concrete -> [ActionCheck m state] -> TestT m ()
...
anyM f = \case
  ...
  x : xs -> do
    y <- f x
    if y then pure True else anyM f xs


linearize :: Monad m => state Concrete -> [ActionCheck m state] -> [ActionCheck m state] -> TestT m ()
linearize initial branch1 branch2 =
  withFrozenCallStack $ do
    ok <-
      lift $ anyM successful $ fmap (checkActions initial) $ interleave branch1 branch2
    if ok then
      pure ()
    else
...
--
executeParallel ::
     (MonadCatch m, MonadBaseControl IO m, HasCallStack)
  => (forall v. state v)
  -> Parallel m state
  -> TestT m ()
